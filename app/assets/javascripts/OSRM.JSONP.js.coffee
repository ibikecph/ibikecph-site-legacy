...
OSRM.JSONP = 
	...
	late: () -> {},  # OSRM.debug.log("[jsonp] reply too late")	
	empty: () -> {} # OSRM.debug.log("[jsonp] empty callback")
	
	
	# init JSONP call
	call: (source, callback_function, timeout_function, timeout, id, parameters) ->
		...
		OSRM.JSONP.timeouts[id] = (response) ->
			try 
...
# 			OSRM.debug.log("[jsonp] timout handling: "+id)
		...
		OSRM.JSONP.callbacks[id] = (response) ->
...
# 			OSRM.debug.log("[jsonp] response handling: "+id+" "+ (OSRM.JSONP.sum[id]/OSRM.JSONP.counter[id]).toFixed(2) )
		...
		jsonp = document.getElementById('jsonp_'+id)
		if(jsonp)
...
# 		if(OSRM.JSONP.counter[id]) OSRM.JSONP.counter[id]++ else OSRM.JSONP.counter[id] = 1OSRM.JSONP.sum[id] = 0
