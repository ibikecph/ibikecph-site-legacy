...
timeoutRouteSimple: () ->
	...
	OSRM.RoutingDescription.showNA( I18n.t("timed_out") )
,
timeoutRoute: () ->
	OSRM.RoutingGeometry.showNA()
...
showRouteSimple: (response) ->
	...
	if response.status == 207
		OSRM.RoutingGeometry.showNA()
		OSRM.RoutingDescription.showNA( I18n.t("computing_route") )
	else
...
_buildCall: () ->
	source = OSRM.DEFAULTS.HOST_ROUTING_URL
...
_updateHints: (response) ->
	hint_locations = response.hint_data.locations
	OSRM.G.markers.checksum = response.hint_data.checksum
	for i in [0...hint_locations.length]
